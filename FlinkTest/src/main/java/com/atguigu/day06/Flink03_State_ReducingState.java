...
public class Flink03_State_ReducingState {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<WaterSensor> reducing = keyedStream.process(new KeyedProcessFunction<String, WaterSensor, WaterSensor>() {
            private ReducingState<WaterSensor> reducingState;

            @Override
            public void open(Configuration parameters) throws Exception {
                reducingState = getRuntimeContext().getReducingState(
                        new ReducingStateDescriptor<WaterSensor>("reducing", new ReduceFunction<WaterSensor>() {
                            @Override
        ...
        env.execute(Flink03_State_ReducingState.class.getName());
    }
}
